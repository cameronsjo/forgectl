...
Proxy values reach exactly two forgectl outputs, both purpose-built and neither a display surface: the `proxy use` shell protocol, and the environment of a harness started with `proxy.launch_profile` set — both described in the [proxy](commands/proxy.md) doc. Capture the shell protocol through the wrapper rather than displaying it. Note what the second one implies: a launched agent can read its own environment, so a proxy URL carrying credentials is readable by the harness and anything it runs.
