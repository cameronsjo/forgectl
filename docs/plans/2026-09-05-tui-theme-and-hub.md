...
- 6c k8s: `internal/k8s/logs.go:253,292` (`severityANSI` → a new `LogsOptions.Styles theme.Styles`; Trace→Dim, Debug→Steel, Info→OK, Warn→Warn, Error/Fatal→Danger), `internal/cli/k8s.go:316`, `logs_test.go`. **There is no `LogsOptions.Out`** — an earlier draft of this task said there was. `Logs` takes `stdout, stderr io.Writer` as *parameters* (`logs.go:68`), wraps each in its own `newBoundedLineWriter` (`:79-80`), closes both, and propagates each close error separately (`:83-90`). So the theme writer is applied by the **caller**, at `internal/cli/k8s.go:316`, to `cmd.OutOrStdout()` and `cmd.ErrOrStderr()` — both, since either can carry styled severity text — and neither the paired `Close()` calls nor the two distinct error messages may be collapsed.
...
- **[Task 6c] The k8s task spec named a field that does not exist.** It said to wrap `LogsOptions.Out`; `LogsOptions` has only `MinLevel` and `Color`, and the writers are `stdout, stderr` parameters on `Logs`, each separately wrapped, closed, and error-propagated. Corrected in place — a fixed spec handed to a dispatched implementer is followed contractually, so a wrong one would have produced a wrong implementation or a collapsed error path. Found by automated review on PR 1.
- **[PR 1] `NO_COLOR` lost to `CLICOLOR_FORCE` on a pipe.** `colorprofile` gates its `NO_COLOR` branch on the destination being a TTY, then evaluates `CLICOLOR_FORCE` unconditionally — so the two together promoted a `NoTTY` profile to ANSI and emitted 12 escape sequences from `forgectl doctor` into a pipe. no-color.org makes `NO_COLOR` absolute, so `colorOut` now drops `CLICOLOR_FORCE` when `NO_COLOR` is set non-empty. Found by automated review on PR 1; not a defect this branch introduced, but one it newly exposed.
- **[Task 7] `review` and `ghostty cheat` are out of the piped-output table, each with a written reason.** The plan listed nine verbs; the table ships with seven. `review`'s styled path needs `review.Source` fixtures rather than a runner, so through a root-level harness it renders no rows and no colour. `ghostty cheat` resolves the real ghostty binary through `lookPath`/`stat` before it ever reaches `deps.Runner`, so `newRoot` cannot fake it — it produced colour on the author's Mac and nothing on both CI runners. Both were found by running the table's positive control **per verb** rather than once; `ghostty cheat` moved to its own test that injects the client with `WithBin`, which is stronger coverage than the table gave it.
- **[Task 6c] Error and Fatal do not share a style.** The plan mapped both to Danger. The raw escapes they replaced had Fatal bold and Error not, so the mapping as written would have flattened a severity distinction while claiming to preserve the look. Error is `Danger` without the bold; Fatal keeps it.
- **[Task 6a, unplanned] `ShouldProbe` had to honour its `Mode` argument.** It was shipped in PR 2a with the parameter named `_`, so `[theme] mode = "light"` still queried the terminal background and the TUI then overwrote the operator's setting with the answer — defeating the one escape hatch documented for the case detection cannot get right. Fixed here, with a `ModeAuto` guard on the `BackgroundColorMsg` handler as a second layer.
...
- **A positive control run once and generalised is not a positive control.** The piped-output table asserts "no escape appeared" for every verb, which a verb rendering nothing satisfies for the wrong reason. Forcing colour on one verb and concluding the other eight can go red left them assumed. Run per verb, the same control found two that could not: `review`, which renders no rows through that harness, and `ghostty cheat`, which needs a binary CI does not have. The cost of the shortcut is exactly proportional to the table's size, and it is invisible — every row stays green either way.
- **A green that depends on the dev machine's installed software reads identically to a real one.** `ghostty cheat` was the verb that leaked hardest in this migration, and it was in the table specifically for that. On this Mac it exercised the leak; on both CI runners it errored out before printing and asserted nothing, for two months of branch history had it landed unnoticed. The tell was available locally — the command shells out to a binary — and was not looked for, because the test passed.
