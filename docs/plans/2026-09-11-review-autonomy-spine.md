...
- **2026-09-12 — Task 2 review round: an unreadable record is a report ROW, and `pr repair` exits 1 on it in `--json` too.** Every counting arm refuses while a record cannot be decoded, so such a record blocks every launch — while inspect printed `no records need repair` and exited 0. Unreadable entries now carry `from_phase`/`outcome` of `unreadable` plus the decode error, `--forget-if-absent` settles one without decoding it, and the exit code is decided once for both output shapes rather than only in the human path. `listLocked` returns the unreadable entries rather than a bare count so the report and the refusal cannot drift. Reality-forced (security review Important 4).
