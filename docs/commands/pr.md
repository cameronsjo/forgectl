...
**Each review window gets the launch environment, not the tmux server's.** A window would otherwise inherit whatever the tmux server held when it started. Instead forgectl passes the same injected environment `forgectl launch` uses — bench telemetry and the `[proxy] launch_profile` values — as `-e KEY=VALUE` arguments to `new-window`. tmux cannot unset a variable this way, so a variable the launch would remove is set empty instead. Those arguments are readable by any local user on the command line, so a URL carrying `user:pass@` or a query string (`?api_key=…`) is refused before the window opens. A secret in a URL's path cannot be told from an ordinary path and is not caught; keep keys out of `otlp_endpoint`, which forgectl treats as non-secret, and give a collector that needs one its key through the harness's own environment or its OTLP headers config. The refusal names the config setting to fix, and `forgectl launch doctor` reports it too, before any review is attempted. forgectl never writes the values down: its debug log and its error text show each entry as `KEY=[redacted]`. A bad `launch_profile` fails the review launch before its window opens; `pr list` and `pr teardown` do not read it and keep working.
