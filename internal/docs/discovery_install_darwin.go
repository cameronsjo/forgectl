...
//
// There is no fallback to unix.Renameat. A filesystem that does not implement
// RENAME_EXCL fails publication instead, because plain rename SUCCEEDS at
// overwriting a live sibling's record — a silent wrong answer in place of a
// loud missing feature.
func installNoReplace(dirFD int, tempName, finalName string) error {
	return unix.RenameatxNp(dirFD, tempName, dirFD, finalName, unix.RENAME_EXCL)
}
