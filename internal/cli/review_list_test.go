...
func TestReviewCmd_KindAndRepoFilters(t *testing.T) {
	...
	run := func(args ...string) []reviewRowJSON {
		t.Helper()
		cmd := newReviewCmdForSources([]review.Source{src}, filepath.Join(t.TempDir(), "r.json"), review.GitHubHost, theme.Theme{})
		var stdout bytes.Buffer
		cmd.SetOut(&stdout)
		cmd.SetErr(new(bytes.Buffer))
...
func TestReviewHelp_NamesNoBakedAccount(t *testing.T) {
	help := newReviewCmdForSources(nil, "", review.GitHubHost, theme.Theme{}).Long
