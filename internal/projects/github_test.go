...
func TestGithubListOrg_UsesArbitraryLoginNotGithubOwner(t *testing.T) {
	const out = `[{"isPrivate":false,"name":"claude-code","sshUrl":"git@github.com:anthropics/claude-code.git"}]`
	fake := &exec.FakeRunner{
		RunFunc: func(name string, args []string) (string, error) { return out, nil },
	}
	...
	if repos[0].Owner != "anthropics" || repos[0].Name != "claude-code" {
		t.Errorf("repo = %+v; want owner=anthropics name=claude-code", repos[0])
	}

	last := fake.Last()
	if len(last.Args) < 3 || last.Args[0] != "repo" || last.Args[1] != "list" || last.Args[2] != "anthropics" {
